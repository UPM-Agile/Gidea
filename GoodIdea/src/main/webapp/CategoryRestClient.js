...
// Model for Category entity
    models.Category = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/GoodIdea/rest/category/",
        idAttribute: 'id',
        defaults: {
            name: ""
        },
        ...
        sync: function(method, model, options) {
            ...
            if (method == 'create') {
                options.url = 'http://localhost:8080/GoodIdea/rest/category/';
            }
    ...
    // Collection class for Category entities
    models.CategoryCollection = Backbone.Collection.extend({
        model: models.Category,
        url: "http://localhost:8080/GoodIdea/rest/category/",
        sync: function(method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function(jqXHR, textStatus, errorThrown) {
...
$(function() {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        details: function(id) {
            if (this.collection) {
                this.category = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.category,
                    // tpl-category-details is template identifier for chosen model element
                    templateName: '#tpl-category-details',
        ...
        create: function() {
            ...
            var self = this;
            var dataModel = new models.Category();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-category-details is a template identifier for chosen model element
                templateName: '#tpl-category-details',
